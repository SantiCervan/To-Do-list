...
const TaskList = ({ tasks, handleToggleTaskCompletion, handleDeleteTask, filter }) => {
  return (
    <div className='max-h-[75vh] sm:max-h-[84vh] overflow-y-scroll flex flex-col transition-all bg-neutral-800 py-4 px-3 sm:px-8 rounded-md'>
      ...
      {tasks.length > 0 ?
        tasks.map((task) => (
          <div key={task.id} className="flex items-center py-3 border-b border-neutral-700">
            <input
              type="checkbox"
            ...
            <button
              className="text-white rounded-md w-8 h-8 flex items-center justify-center text-2xl hover:bg-red-900 transition-colors"
